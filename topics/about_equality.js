...
test("numeric equality", function() {
    equal(3 + 4,  7, ""); //4 | Not ('4', "4")
});

test("string equality", function() {
    equal("3" + "7", "37", "concatenate the strings"); // "7", '7', 7, +7 | Not (-7,"+7","-7")
});

test("equality without type coercion", function() {
    ok(3 === 6/2, 'what is exactly equal to 3?'); //3, +3,1+2,2+1,3+0,0+3,6/2,... | Not ("3", '3', -3) 
});

test("equality with type coercion", function() {
    ok(3 == "3", 'what string is equal to 3, with type coercion?'); //3,+3 | Not (1+2, -3)
});
